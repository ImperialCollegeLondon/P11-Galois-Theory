...
import .algebra ring_theory.principal_ideal_domain ring_theory.ideal_operations
...
variables [algebra F K]

theorem algebra.eq_zero_iff {x : F} : algebra.f K x = 0 ↔ x = 0 :=
⟨λ h, is_ring_hom.inj F (algebra.f K) $ h.symm ▸ (is_ring_hom.map_zero _).symm,
